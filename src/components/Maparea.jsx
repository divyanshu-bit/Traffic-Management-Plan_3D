...
const MapArea = ({
  ...
  useEffect(() => { onAssetRemoveRef.current = onAssetRemove; }, [onAssetRemove]);
  ...
  const ensureRoadsNear = useCallback(async (lat, lng) => {
    ...
    if (roads?.features?.length) { setRoadCollection?.(roads); lastFetchRef.current = { lat, lng }; return roads; }
    ...
    if (activeTool?.startsWith('draw-')) {
      ...
      if (activeTool === 'draw-rectangle' && draftCoordsRef.current.length === 2) {
        ...
        if (mapRef.current) {
          ...
          map.getSource('draft-shape')?.setData({ type: 'FeatureCollection', features: [] });
          map.getSource('cursor-source')?.setData({ type: 'FeatureCollection', features: [] });
        }
        setActiveTool?.(null);
        return;
      }
      ...
      if (clicked) {
        ...
        if (zoneId === activeZoneId && clickedZone && clickedZone.coords?.length > 1) {
          ...
          for (let i = 0; i < loopLimit; i++) {
            ...
            if (dist < minDistance) { minDistance = dist; closestEdgeIndex = i; }
          }
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    requestRef.current = requestAnimationFrame(() => {
      ...
      if (draftSource && draftCoordsRef.current.length > 0) {
        draftSource.setData(featureFromCoords(draftCoordsRef.current, toolToShapeType(activeTool), cursorPt));
      }
      map.getSource('cursor-source')?.setData({
        type: 'Feature',
  ...
  // Re-order layers after style changes
  useEffect(() => {
    ...
    const reorder = () => {
      try {
        const layerOrder = [
          ...
          'approach-sides-glow', 'approach-sides-dash',
          'draft-fill', 'draft-line',
          'cursor-layer',
          TRAFFIC_THREE_LAYER_ID,
        ];
  ...
  // Create / re-add 3D layer when map instance is available
  useEffect(() => {
    ...
    const ensureTrafficLayer = () => {
      ...
      try {
        if (!map.getLayer(TRAFFIC_THREE_LAYER_ID)) map.addLayer(trafficLayerRef.current);
        trafficLayerRef.current.setData(trafficDataRef.current);
      } catch (err) {
        console.warn('[3D] addLayer failed (style mid-switch?):', err.message);
      }
    };
    ...
    return () => {
      ...
      try {
        ...
        if (map.getLayer?.(TRAFFIC_THREE_LAYER_ID)) map.removeLayer(TRAFFIC_THREE_LAYER_ID);
      } catch (_) { }
  ...
  // Keep 3D layer in sync with React state
  useEffect(() => {
  ...
  // Track first symbol layer for satellite raster insertion
  useEffect(() => {
    if (!mapInstance) return;
    const map = mapInstance;
    const updateFirstLayer = () => {
      try {
        ...
        if (layers?.length) {
          ...
          if (firstSymbol) setFirstStyleLayerId(firstSymbol.id);
        }
      } catch (_) { }
    };
    updateFirstLayer();
  ...
  useEffect(() => {
    if (!activeTool?.startsWith('draw-') && mapRef.current) {
      const map = mapRef.current.getMap();
      map.getSource('draft-shape')?.setData({ type: 'FeatureCollection', features: [] });
      map.getSource('cursor-source')?.setData({ type: 'FeatureCollection', features: [] });
    }
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', pointerEvents: isExporting ? 'none' : 'auto' }}>
      <Map
        ...
        // REQUIRED: share the WebGL context with Three.js and enable antialiasing
