...
const Sidebar = ({
  ...
  const buildPDF = async () => {
    ...
    // ── PAGES 2+: ONE MAP PAGE + ONE MANIFEST PAGE PER ZONE ─────────────────
    let pageNum = 2;
    ...
    for (const zone of zones) {
      ...
      if(mapEl){
        try{
          const canvas=await html2canvas(mapEl,{scale:2,useCORS:true,allowTaint:false,backgroundColor:'#0f172a',logging:false});
          const img=canvas.toDataURL('image/jpeg',0.92);
          const mapH=Math.min(H-curY-28,(canvas.height/canvas.width)*UW);
