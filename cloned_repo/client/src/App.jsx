...
const App = () => {
  ...
  const firstZone = useRef(makeZone(0)).current;
  const [zones, setZones]               = useState([firstZone]);
  const [activeZoneId, setActiveZoneId] = useState(firstZone.id);
  const [isWazeSync, setIsWazeSync]           = useState(false);
  ...
  const [workingHours, setWorkingHours] = useState('07:00–17:00');
  ...
  const pushUndo = useCallback((snap) => { history.current.push(JSON.parse(JSON.stringify(snap))); if (history.current.length > 20) history.current.shift(); redoStack.current = []; }, []);
  const handleUndo = useCallback(() => { if (!history.current.length) return; redoStack.current.push(JSON.parse(JSON.stringify(zones))); setZones(history.current.pop()); }, [zones]);
  const handleRedo = useCallback(() => { if (!redoStack.current.length) return; history.current.push(JSON.parse(JSON.stringify(zones))); setZones(redoStack.current.pop()); }, [zones]);

  const canUndo = history.current.length > 0;
  const canRedo = redoStack.current.length > 0;
  const activeZone = zones.find(z => z.id === activeZoneId) || zones[0] || null;

  const updateActiveZone = useCallback((patch) => { setZones(prev => prev.map(z => z.id === activeZoneId ? { ...z, ...patch } : z)); }, [activeZoneId]);
  const handleAddZone = useCallback(() => { const z = makeZone(zones.length); pushUndo(zones); setZones(prev => [...prev, z]); setActiveZoneId(z.id); setActiveTool(null); showToast(`${z.name} added`); }, [zones, pushUndo, showToast]);
  const handleDeleteZone = useCallback((id) => { if (zones.length === 1) return showToast('At least one zone is required'); pushUndo(zones); const remaining = zones.filter(z => z.id !== id); setZones(remaining); if (activeZoneId === id) setActiveZoneId(remaining[0].id); showToast('Zone deleted'); }, [zones, activeZoneId, pushUndo, showToast]);
  const handleRenameZone = useCallback((id, name) => { setZones(prev => prev.map(z => z.id === id ? { ...z, name } : z)); }, []);
  const handleSelectZone = useCallback((id) => { setActiveZoneId(id); setActiveTool(null); }, []);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (d.zones?.length) { setZones(d.zones); setActiveZoneId(d.activeZoneId || d.zones[0].id); }
        setProjectName(d.projectName || ''); setPermitNumber(d.permitNumber || '');
        setContractorName(d.contractorName || ''); setClientName(d.clientName || '');
        setStartDate(d.startDate || ''); setEndDate(d.endDate || '');
        setSuperintendent(d.superintendent || ''); setSafetyOfficer(d.safetyOfficer || '');
  ...
  }, [zones, activeZoneId, projectName, permitNumber, contractorName, clientName, startDate, endDate, superintendent, safetyOfficer, emergencyContact, isWazeSync]);

  const handleToolSelect = useCallback((tool) => { setActiveTool(tool); if (tool?.startsWith('draw-')) setDrawSessionKey(k => k + 1); }, []);
  const handleClear = useCallback(() => { if (!activeZone) return; pushUndo(zones); updateActiveZone({ coords: [], placedAssets: [], hasGenerated: false }); showToast('Zone cleared'); }, [activeZone, zones, pushUndo, updateActiveZone, showToast]);
  const handleAssetRemove = useCallback((id) => { setZones(prev => prev.map(z => z.id === activeZoneId ? { ...z, placedAssets: z.placedAssets.filter(a => a.id !== id) } : z )); }, [activeZoneId]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setTimeout(() => {
      pushUndo(zones);
      setZones(prev => prev.map(z => z.id === activeZoneId ? { ...z, placedAssets: [...z.placedAssets.filter(a => a.source !== 'auto'), ...autoCones], hasGenerated: true } : z));
  ...
  }, [activeZone, activeZoneId, zones, pushUndo, showToast, roadCollection]);
  ...
  const handleShapeDrawn = useCallback((coords, type) => {
    ...
    setZones(prev => prev.map(z => z.id === activeZoneId ? { ...z, coords, shapeType } : z));
    setActiveTool(null); setDrawPointCount(0);
    showToast(`${activeZone?.name || 'Zone'} boundary set`);
  }, [activeZoneId, activeZone, showToast]);

  const handleSetPlacedAssets = useCallback((updater) => {
    setZones(prev => prev.map(z => z.id === activeZoneId ? { ...z, placedAssets: typeof updater === 'function' ? updater(z.placedAssets) : updater } : z ));
  }, [activeZoneId]);

  const handleUpdatePointCount = useCallback((count) => setDrawPointCount(count), []);
  ...
  return (
    <div className="app-container">
      ...
      <MemoizedSidebar
        ...
        zones={zones} activeZoneId={activeZoneId} activeZone={activeZone}
        onSelectZone={handleSelectZone} onAddZone={handleAddZone}
        onDeleteZone={handleDeleteZone} onRenameZone={handleRenameZone}
        updateActiveZone={updateActiveZone}
        isWazeSyncEnabled={isWazeSync}      setIsWazeSyncEnabled={setIsWazeSync}
        ...
        nightWork={nightWork}               setNightWork={setNightWork}
        superintendent={superintendent}     setSuperintendent={setSuperintendent}
        safetyOfficer={safetyOfficer}       setSafetyOfficer={setSafetyOfficer}
        emergencyContact={emergencyContact} setEmergencyContact={setEmergencyContact}
        onGenerate={handleGenerate} isGenerating={isGenerating} genProgress={genProgress}
        reportId={reportId.current} saveStatus={saveStatus}
      />
      ...
      <MemoizedFloatingDock
        activeTool={activeTool} onSelectTool={handleToolSelect}
        ...
        drawPointCount={drawPointCount} activeZone={activeZone}
